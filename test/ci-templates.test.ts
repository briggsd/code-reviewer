...
describe("CI starter templates", () => {
  test("GitHub Actions template separates read-only dry run from guarded write-back", async () => {
    ...
    expect(workflow).toContain('AI_REVIEW_PACKAGE: "@briggsd/code-reviewer@0.5.0"');
  ...
  test("GitLab CI template separates MR dry run from same-project write-back", async () => {
    ...
    expect(pipeline).toContain(
      "AI_REVIEW_PACKAGE: https://gitlab.example.com/api/v4/projects/<project-id>/packages/generic/code-reviewer/0.5.0/code-reviewer.tgz",
    );
    expect(pipeline).not.toContain('AI_REVIEW_PACKAGE: "@briggsd/code-reviewer@0.5.0"');
