...
describe("preservation of existing behaviors", () => {
  ...
  test("hidden metadata block is omitted when includeHiddenMetadata is false", () => {
    ...
    expect(markdown).not.toContain("<!-- code-reviewer");
  });
  ...
  test("HTML-comment breakout: findingTitles value with '-->' is unicode-escaped and round-trips", () => {
    ...
    const metadataBlock = /<!-- code-reviewer\n([\s\S]*?)\n-->/.exec(markdown)?.[1] ?? "";
  ...
  test("break-glass footer always present before _Generated by_ line", () => {
    ...
    expect(markdown).toContain(
      "[Break-glass / human override](https://github.com/briggsd/code-reviewer/blob/main/docs/developer/architecture.md#break-glass--human-override)",
    );

    // Footer must appear before the _Generated by_ line
    const footerIdx = markdown.indexOf("🔓 Break glass");
    const generatedIdx = markdown.indexOf("_Generated by code-reviewer._");
    expect(footerIdx).toBeLessThan(generatedIdx);
  });

  test("_Generated by code-reviewer._ line always present", () => {
    const markdown = formatReviewSummaryMarkdown(makeSummary({ findings: [] }));
    expect(markdown).toContain("_Generated by code-reviewer._");
  });
  ...
  test("hidden metadata block always last when present", () => {
    ...
    const generatedIdx = markdown.indexOf("_Generated by code-reviewer._");
    const metaIdx = markdown.indexOf("<!-- code-reviewer");
...
describe("runStats — footer stats and low-activity warning (#285)", () => {
  test("footer with runStats folds stats into the generated-by line", () => {
    ...
    expect(markdown).toContain(
      "_Generated by code-reviewer · Review: 175s · 5 reviewers · ~47.2k model tokens._",
    );
    // Must NOT contain the bare line when runStats is present
    expect(markdown).not.toContain("_Generated by code-reviewer._");
  });
