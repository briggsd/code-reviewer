...
Risk tier also bounds runtime effort. The configured timeout values are full-review ceilings; the default full-tier ceiling is 15 minutes, with 6 minutes per reviewer attempt, 4 minutes for coordinator synthesis, and 5 minutes of wall-clock headroom. Lite reviews use half of those reviewer/coordinator/overall budgets, and trivial reviews use one quarter. Retryable reviewer failures are retried only when the remaining effective tier budget can still cover another reviewer attempt, the coordinator budget, and the retry reserve; the reserve is scaled by the same risk tier as those budgets so retry headroom stays proportional on lite and trivial runs instead of an unscaled floor swallowing the shrunken budget. Tool policy is tightened by tier as well: lite and trivial reviewers rely on supplied diff/context artifacts and deny repo-crawling read tools (`read`, `grep`, `find`, `ls`) plus shell/write tools (`bash`, `write`, `edit`), while full reviews may use read/grep/find/ls when the selected safety mode allows them.
