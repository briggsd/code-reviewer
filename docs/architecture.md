...
**Known failure mode — all reviewers fail.** If *every* dispatched reviewer fails (e.g. the agent runtime cannot produce parseable structured output), the coordinator has nothing to synthesize and re-throws the first failure ("All selected reviewers failed before coordinator synthesis"). The run then exits non-zero and **posts no summary at all** — there is currently no degraded "review incomplete" comment for this case (unlike the timeout partial above). In CI this surfaces as the publish job failing; operators should treat an `agent.failed` on every reviewer as *no review performed*, not a passing review. To triage, look for `All selected reviewers failed before coordinator synthesis` in the coordinator job log (or `agent.failed` events on every reviewer step). Because model generation is non-deterministic, re-running the workflow is safe and often succeeds for content-triggered parse failures (the same diff can parse on a later attempt), so a single such failure does not by itself indicate a persistent infrastructure problem. Degrading this to a posted notice under the fail-open/closed policy is a tracked robustness follow-up.
