...
- PR #9 verified same-repository real Pi review publishing after enabling `AI_REVIEW_REAL_REVIEW_ENABLED=true` and configuring `ANTHROPIC_API_KEY`: dummy dry-run passed, dummy summary publish skipped, real Pi summary publish succeeded with the default model (`claude-sonnet-4-6` at the time of this run), artifacts uploaded as `ai-review-real-9`, and inline publishing was not invoked because the default workflow omits `--publish-inline`.
