...
- PR #4 verified the M004 inline publishing smoke criteria with a seeded finding on `src/cli/run-options.ts:9`: only readiness-approved findings produced inline review comments, skipped findings stayed visible in the summary and trace output, rerunning on the same head reported `duplicate_inline_comment`, and the summary comment was updated instead of duplicated.
- PR #9 verified same-repository real Pi review publishing after enabling `AI_REVIEW_REAL_REVIEW_ENABLED=true` and configuring `ANTHROPIC_API_KEY`: dummy dry-run passed, dummy summary publish skipped, real Pi summary publish succeeded with the default `anthropic`/`claude-sonnet-4-6` model, artifacts uploaded as `ai-review-real-9`, and inline publishing was not invoked because the default workflow omits `--publish-inline`. That run motivated raising live-review defaults to `reviewerMs: 360000`, `coordinatorMs: 240000`, and `overallMs: 660000` (11 minutes); later full-tier dogfooding raised the default `overallMs` to `900000` (15 minutes).
