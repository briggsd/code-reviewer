...
3. **Deterministic orchestration, agentic judgment.** Code controls fetching, filtering, fan-out, timeouts, retries, state, and publishing. Agents judge code risks inside bounded contracts. Any deterministic step that discards or suppresses reviewer output must carry a drop-rate signal, be visible at the moment of the drop, and have a labeled regression case (M020 S01).
...
`change-context.json` contains run metadata, risk, prior state, and changed-file metadata with `patchPath` references for files that have a written patch body. `patchPath` is conditional: it is absent for binary/empty-patch files, for fully-deleted files, and for files whose entire diff is deletion-only (both pruned from review context per #144 — the `context.built` trace records `deletionHunksPruned`/`deletedFileBodiesPruned` counts). Those files still appear by name and stat. It intentionally omits inline patch bodies and never carries full PR/MR-head file bodies. Patch files are written once under `patches/` using deterministic safe names. Reviewer inputs carry `contextReferences` pointing at the shared context and assigned patch files; Pi reviewer prompts prefer those references and only fall back to inline diff payloads when read tools are unavailable.
...
- **Operational failures** (the review could not run — `provider_error`/quota-billing, `auth`, `rate_limited`, `retryable_transient`, `timeout`, `unsafe_fork`) keep **crashing the run loudly** (non-zero exit, no summary posted, `review.failed` trace with the error classification). This is deliberate: silently degrading an outage to a `review_failed` notice would let a fail-**open** repo pass merges through during a billing/auth incident. An infrastructure failure should alarm, not fail-open. **This holds even when the model provider returns the error mid-turn** rather than failing the subprocess: a Pi turn ending in `stopReason:"error"` is detected (`assertNoTerminalModelError`) and routed into the same operational-failure path (#283) — so a provider 4xx (e.g. credit/quota exhaustion) makes the review CI step **fail red**, never publish a silent `✅ Approved`. *Operator note:* an unexpected red review check during a model-provider outage is this guard working — resolve the provider issue and re-run; do not disable or mark the check non-required.
...
**Evidence-grounding reframe — down-weight, don't drop (#207/#214).** The deterministic grounding step (`src/runner/evidence-grounding.ts`) partitions findings into `grounded` (cited code found in the changed file's grounding corpus) and `dropped` (cited code not found). The corpus is per changed file: patch hunks remain the safe floor, including deleted lines, and an optional PR/MR-head full-file corpus promotes findings that quote real code from unchanged regions of that same changed file. Full-file entries are admitted only for files still present in the filtered/reviewed diff, are bounded by the same per-tier patch budget, and rank signal-bearing source files before low-signal bulk. A `dropped` finding is **not silently discarded** — it is down-weighted to `confidence: "low"` and kept visible in a labeled "Low-confidence findings (kept, non-blocking)" render block. It is excluded from the CI gate, the title count, and stable-id assignment, but it is always shown so no valid finding is ever silently lost. Findings with no quotable code line (`quotedCode` undefined/empty, or only sub-threshold quotes) are never demoted — they carry no fabricated-location risk and ground at full confidence, remaining blocking-eligible. Truncated diffs keep the conservative safety behavior: grounding does not demote on a partial corpus.
