...
3. **Deterministic orchestration, agentic judgment.** Code controls fetching, filtering, fan-out, timeouts, retries, state, and publishing. Agents judge code risks inside bounded contracts. Any deterministic step that discards or suppresses reviewer output must carry a drop-rate signal, be visible at the moment of the drop, and have a labeled regression case — see [Suppression-observability principle](suppression-observability.md) (M020 S01).
